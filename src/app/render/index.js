...
export function buildApp() {
  ...
  const sched   = S.tasks.filter(t => t.startWeek !== null).length;
...
${ro ? `<div class="ro-banner">
  ...
  <span>Bản xem — chỉ đọc, không thể chỉnh sửa</span>
...
<div class="hdr">
  <div class="hdr-row1">
    ${currentProjId && !ro
      ? `<div class="breadcrumb">
          <button class="bc-home" id="btn-back-home" aria-label="Back to projects">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
            Projects
          </button>
          <span class="bc-sep" aria-hidden="true">›</span>
          <div class="bc-cur-wrap" id="hdr-title-click" role="button" tabindex="0" aria-label="Edit project name">
    ...
    <div class="hdr-btns">
      ${ro ? '' : `
      <button class="btn btn-ghost" id="btn-cfg">${svgIcon('settings', 13)} Settings</button>
      <div class="btn-dd" role="none">
        ...
        <div class="add-dd-menu" id="add-new-menu" role="menu">
          <button class="add-dd-item" role="menuitem" data-add="phase">${svgIcon('map', 13)} Phase</button>
          <button class="add-dd-item" role="menuitem" data-add="add-task">${svgIcon('edit', 13)} Task</button>
          <button class="add-dd-item" role="menuitem" data-add="add-team">${svgIcon('users', 13)} Team</button>
          <div style="height:1px;background:var(--bd);margin:3px 5px"></div>
          <button class="add-dd-item" role="menuitem" data-add="import-csv"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg> Import CSV</button>
        </div>
      </div>
      <button class="btn btn-gold" id="btn-export">${svgIcon('package', 13)} Export PDF</button>
      <button class="btn btn-ghost btn-outline-gold" id="btn-share" aria-label="Share roadmap">
  ...
  <div class="hdr-row2" role="group" aria-label="Stats">
    <button class="stat" id="stat-all" title="Show all tasks" aria-label="${total} total tasks"><span class="stat-n gd">${total}</span><span class="stat-l">Total</span></button>
    <button class="stat" id="stat-sched" title="Filter scheduled tasks" aria-label="${sched} scheduled tasks"><span class="stat-n gn">${sched}</span><span class="stat-l">Scheduled</span></button>
    <button class="stat" id="stat-back" title="Filter backlog" aria-label="${unsched} backlog tasks"><span class="stat-n mt">${unsched}</span><span class="stat-l">Backlog</span></button>
    <div class="stat" aria-label="${tw} weeks"><span class="stat-n" style="color:var(--txt2)">${tw}</span><span class="stat-l">Weeks</span></div>
  </div>
</div>
